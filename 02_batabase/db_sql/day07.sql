...
-- 다양한 집계함수
select min(rating), max(movie_name)
...
-- 개수가 많은 것부터, 갯수가 같다면 개봉연도가 최신부터
select rating, substr(open_date,1,4) as Y, count(*) as cnt
from   movie_n
group by rating,Y
order by cnt desc, Y desc
;
