...
//		integer score1 = new integer(100);
		...
		for(int i=0;i<score2.length;i++) {
			MyUtil.p(stdName2[i] + " : " + score2[i] + "점");
		}
		...
		// Enganced For를 활용한 Difficulty GOD 문제
		// String[][] classStd = {{"A","B"},{"C","D"}};
		// Enganced For를 활용하여 모든 학생을 출력하시오
		for(int i=0;i<classStd.length;i++) {
			for(String std : classStd[i]) {
				MyUtil.p(std);
			}
